...
class DataBagger:
    def __init__(self):
        ...
        with dai.Device(pipeline) as device:
            ...
            # bag1 = rosbag.Bag('./data_'+str(int(math.floor(time.time())))+"_1.bag", 'w') 
            # bag2 = rosbag.Bag('./data_'+str(int(math.floor(time.time())))+"_2.bag", 'w')
            # print('bag created')
            if self.use_papilarray:
            ...
            while not rospy.is_shutdown():
                ...
                if self.collect_data_flag: ##Start data collection
                    ...
                    data = tactile_data_to_df(self.tactile_data_0[1], self.tactile_data_1[1])  
                    ...
                    angular_vel = output[:,:,0].item() * 400

                    self.lstm_pub.publish(angular_vel)
                    pred_pos = output[:,:,1].item() * 90

                    time_delay = 0

                    # future predict the angle of the object, offset by time delay
                    # object_angle = self.current_angle + angular_vel * time_delay
                    # goal = 90
