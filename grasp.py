...
class GraspExecutor:
    ...
    def callback(self,data):
        initial_guess = [-0.598, 0.020, 0.224]#[-0.648, -0.047, 0.458]
        threshold = 0.15
        max_angle = 90
        final_grasp_pose = 0
        ...
        for g in data.grasps:
            ...
            # print("camera link exists" if self.tf_listener_.frameExists("/camera_link") else "uh oh no camera")

            # print("base_link exists" if self.tf_listener_.frameExists("/base_link") else "uh oh no base_link")
            ...
            # min_dist = min(min_dist, dist_to_guess(p_base.pose.position, initial_guess))
            # dist_temp = dist_to_guess(p_base.pose.position, initial_guess)
            # print(dist_temp)
            ...
            if theta_approach < max_angle:
                ...
                if plan_offset.joint_trajectory.points and plan_to_final.joint_trajectory.points:
